...
|---|---|
| **线条** | 实心色条：行间分隔线，或整块底色 |
| **边框** | 沿边缘的轮廓，厚度可设（默认 0,3 mm） |
| **文字旋转** | `0°`、`90°`、`180°`、`270°` —— 用于文字横排的线缆标签 |

旋转元素的宽度（`w`）是**文字的长度**，不是框宽：旋转后它变成块的高度。设计器
画布会如实显示旋转：90° 和 270° 时宽高互换。

**贴边吸附。** 拖动时元素不仅吸附到 0,5 mm 网格，还会吸附到标签的**边缘和
中心** —— 画布上会出现红色提示线。按住 **Alt** 可关闭贴边吸附，以便凭眼摆放。

### 由 SMILES 生成化学结构

元素种类 **`smiles`**：一段 SMILES 字符串、以毫米计的尺寸和键线粗细。
...
SMILES 也可以来自数据：`"smiles": "{4}"` 会取记录的第四列，这样整批标签各自带上
自己的结构。

**由 RDKit 绘制。** 我们不自己写结构渲染器：解析 SMILES 只是一半工作，而二维排版
（环的识别、坐标生成、避免重叠）是 RDKit 多年积累的成果。驱动调用它并取回现成的 PNG。

#### 哪些是我们的，哪些不是

结构是驱动**唯一**依赖第三方代码的地方。分界如下：

| 我们的驱动（Go） | RDKit（Python） |
|---|---|
| 模板中的 `smiles` 元素种类 | 解析 SMILES 字符串 |
| 以毫米计的尺寸、旋转、键线粗细 | 二维排版：环、坐标、避免重叠 |
| 代入数据列：`"smiles": "{4}"` | 把分子绘制成 PNG |
| 缓存：同一分子在一批里只画一次 | |
| 查找带 RDKit 的 Python、给出清楚的错误 | |

**为什么不自己写。** 解析 SMILES 只是一半工作，大约一周。真正的难点是**二维排版**：
判断环在哪里，如何摆放原子使键线既不交叉也不重叠。RDKit 和 Open Babel 为此花了
多年，自己实现只会明显更差。

**许可证。** RDKit 核心是 **BSD 3-Clause**，PyPI 上的 `rdkit` 包装是 **MIT**。两者都是
宽松许可，与我们的 MIT 兼容。RDKit **不随本驱动分发**：它单独安装，因此不影响驱动的
许可证。
...
不需要 root。驱动按以下顺序查找 Python：环境变量 `NIIMBOT_RDKIT`、程序旁的
`chemvenv`、`~/.local/share/niimbot/chemvenv`，最后是系统的 `python3`。没有 RDKit 时，
打印结构会明确报错，其余功能照常工作。

#### 尺寸上的要点

14×30 的标签只有 **240×96 点**。结构必须**按所需尺寸**作画并使用**较粗的键线**：
下载来的结构图缩到 96 点会碎成斑点。实测：小分子（9–15 个重原子）在 96×96 的
方块里可以辨认；大分子（胆固醇，28 个）只有**占满标签宽度**时才读得出来。杂原子
标记（O、N）在每毫米 8 点的条件下会丢失 —— 一个字母约占 6 点。

#### 旋转结构

标签只有 12 mm 高，方形结构会顶到这个上限。旋转解除了这个限制：元素有 `rotate`
字段，旋转后的结构可以占用标签的**整条长度**。**由 RDKit 自己旋转**（它的
`rotate` 选项）—— 它按给定的框来排版分子，而不是先塞进正方形再旋转。旋转对图片
同样有效。

**但存在极限，而且在纸上。** 14×30 的标签放不下又大又多的文字：把结构旋转到
18×11 mm 后结构变大了，但文字栏变窄，名称被截断。最终采用**结构 15×10 mm、
文字栏 14 mm** —— 结构够大，文字也完整。

#### 现成的批量模板

设置里保存了模板**「Парфюмерные ингредиенты」（香水原料）**。数据是 CSV，列为
名称;CAS;SMILES;香调。结构取自第 3 列，名称取自第 1 列（加粗），CAS 取自第 2 列
（始终打印），香调取自第 4 列，日期和时间在打印时填入。

### 黑白阈值

打印机**只打印黑白**，因此灰度图像在发送前按阈值转换：比阈值更暗的变为黑色。
默认值是 255 中的 **200**，这比看上去更重要。线条和文字带有抗锯齿，边缘是浅灰的；
在阈值 128 时，结构的键线会变成**虚线**，CAS 号里的数字会碎成点。纸上验证：128 会
断裂，**200** 得到密实的线条，230 以上字母开始糊在一起。

可以通过 `--threshold` 参数、设计器里的「黑白阈值」字段，以及 `/api/preview` 和
`/api/print` 中的 `threshold` 字段调整。**设计器的预览现在直接显示黑白效果** ——
正是送进打印机的内容。

### 打印日期与时间

任何文字元素都接受在**打印瞬间**替换的字段：
...
设计器里有 **「+ 日期与时间」** 按钮，会在下边缘放一个小号文字元素。做原料标签很
方便：能看出混合料是何时配制的。

### 在设计器中保存模板

1. 在画布上搭好标签 —— 文字、图片、线条、结构。
2. 在**「模板」**区输入名称，例如 `Драхма Парфия`。
3. 按**「保存模板」**，按钮下方会出现确认提示。

模板同时保存**元素**（什么放在哪里、用什么字体和字号）和**打印设置**：长度、浓度、
标签类型、份数、阈值。要回到它，在**「加载已保存的」**列表中选择即可。旁边的按钮
分别用于删除所选模板和清空画布。
...
- **边框、分隔线和 90° 文字旋转** —— 一起打印在同一张标签上：边框四边完整，
  分隔线平直，旋转的文字歪头即可阅读；
- **打印浓度** —— 三档（1、2、3）都能得到可用的效果，包括 0,2 mm 的细线和
  1,6 mm 的小字；其中 **2** 被评定为最好，因此作为默认值；
...
**化学结构**由 **[RDKit](https://www.rdkit.org/)**（BSD 3-Clause）绘制 —— 它作为单独的程序调用，不随驱动分发。本驱动的其余部分都是自己的 Go 代码。
